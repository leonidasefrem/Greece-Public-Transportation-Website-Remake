<?php
    session_start();
    if(isset($_GET['dromologio'])){
        $_SESSION['dromologio']=$_GET['dromologio'];
    }
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="container ">
        ...
        <div class="row ">
            <div class="col jumbotron">
                <h2>Επιλέξτε την γραμμή που επιθυμίτε</h2>
                <hr>
                <form method="get" action="emfanisi_dromologiou.php" class=form-group>
                    <?php
                        ...
                        if(isset($_GET['source']) && isset($_GET['destination'])) {
                            ...
                            $query ="SELECT * FROM bus_stops WHERE (stop_1='$source' OR stop_2='$source' OR stop_3='$source' OR stop_4='$source' OR stop_5='$source') AND (stop_1='$destination' OR stop_2='$destination' OR stop_3='$destination' OR stop_4='$destination' OR stop_5='$destination')";
                            $result=mysqli_query($conn,$query);
                       
                            while($row=mysqli_fetch_array($result)){
                                $id=$row['bus_route_id'];
                                echo '<td><input type="radio" name="radioans" value="' . $id . '"/>
                                    <label class="form-check-label " for="exampleRadios1" name="radioans">
                                            ' . $row['bus_route_id'] . '  ' . $row['line_name'] . ' 
                                </label></td>';
                                echo '<br>';
                                echo '<br>';
                            }
                            if (mysqli_num_rows($result)==0) {
                                echo '<p style="color:red;">Δεν υπάρχουν δρομολόγια για την διαδρομή που επιλέξατε</p>';
                                echo '<br>';
                             }else{
                                echo '<button type="submit" class="btn btn-primary">Υποβολή</button>';  
                             }
                            CloseCon($conn);
                                                 }
                        ?>
